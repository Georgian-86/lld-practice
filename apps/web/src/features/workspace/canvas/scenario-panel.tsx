...
import { AlertTriangle, ArrowRight, CheckCircle2, CornerDownRight, MousePointerClick, Plus, Trash2, Undo2, X } from 'lucide-react';
    ...
    <>
      <div className="flex h-12 shrink-0 items-center justify-between gap-2 border-b border-border px-4">
        <div className="min-w-0">
          ...
          <div className="truncate text-[11.5px] text-muted">Show the calls that fulfil a requirement</div>
        </div>
            ...
            {analysis && analysis.steps.length > 0 && (
              <div
                className={cn(
                  ...
                  analysis.valid ? 'text-success' : 'text-warning',
                )}
                role="status"
              >
                ...
                return (
                  <li key={step.id} className={cn('rounded-lg border p-2', bad ? 'border-danger/40 bg-danger-soft/40' : 'border-border')}>
                    <div className="flex items-center gap-2">
                      <span className={cn('inline-flex size-5 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold text-white', bad ? 'bg-danger' : 'bg-ai')}>
                        {i + 1}
                      </span>
                    ...
                    {(a?.problems.length || broken) && (
                      <ul className="mt-1.5 space-y-0.5 text-[11.5px] text-danger-soft-fg">
                        {a?.problems.map((p, k) => <li key={k}>{describeStepProblem(p, step)}</li>)}
